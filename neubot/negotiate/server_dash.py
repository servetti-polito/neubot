# neubot/negotiate/server_dash.py
...
class NegotiateServerDash(NegotiateServerModule):

    ''' Negotiator for Dash '''

    def __init__(self):
        ''' Initialize Dash negotiator '''
    ...
    def collect_legacy(self, stream, request_body, request):
        ...
        if ident not in self.clients:
            ...
            # for dash, which were used both for testing and for
            ...
            if authorization not in self.clients:
                ...
                logging.warning('dash: working around multiple conns '
                                'issue')
                ident = authorization
        ...
        if privacy.collect_allowed(request_body):
            BACKEND.dash_store(request_body)
        else:
...
NEGOTIATE_SERVER_DASH = NegotiateServerDash()
