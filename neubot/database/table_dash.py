...
CREATE_TABLE = _table_utils.make_create_table("dash", TEMPLATE)
INSERT_INTO = _table_utils.make_insert_into("dash", TEMPLATE)

def create(connection, commit=True):
    ''' Create a new dash table '''
    connection.execute(CREATE_TABLE)
    if commit:
        connection.commit()

def insert(connection, dictobj, commit=True, override_timestamp=True):
    ''' Insert a result dictionary into dash table '''
...
def listify(connection, since=-1, until=-1):
    ''' Converts the content of dash table into a list '''
    vector = []
    cursor = connection.cursor()
    query = _table_utils.make_select("dash", TEMPLATE,
...
def prune(connection, until=None, commit=True):
    ...
    connection.execute("DELETE FROM dash WHERE timestamp < ?;", (until,))
    if commit:
        connection.commit()
